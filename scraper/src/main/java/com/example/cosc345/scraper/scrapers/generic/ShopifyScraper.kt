...
abstract class ShopifyScraper(
    ...
    private val whiteListedCategories: List<String>? = null
) : Scraper() {
    override suspend fun runScraper(): ScraperResult {
        ...
        while (lastSize == 250) {
            ...
            response.products.forEach { shopifyProduct ->
                if (whiteListedCategories == null || whiteListedCategories.contains(shopifyProduct.productType)) {
                    ...
                    product.name = titleFormatted.trim().lowercase().titleCase().capitaliseNZ()
